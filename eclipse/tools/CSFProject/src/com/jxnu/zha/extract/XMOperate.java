...
public class XMOperate {
	 ...
	 * 实例
	 */
	private volatile static XMOperate instance;
    /**
     * 构造函数
     */
	private XMOperate(){
//        System.out.println("XMOperate Singleton has loaded-----------------------------------------");
    }
    /**
     * 获取实例
     * @return
     */
	public static XMOperate getInstance(){
        if(instance==null){
            synchronized (XMOperate.class){
                if(instance==null){
                    instance=new XMOperate();
                }
            }
        }
        return instance;
	...
	public void classifyProvince(){
		String result = FileUtil.readFileData(Config.XMPath);
		result = result.replaceAll(",", splitStr);
